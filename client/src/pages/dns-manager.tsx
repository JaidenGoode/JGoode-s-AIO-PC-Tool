...
export default function DNSManager() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        title: "DNS Updated",
        description: `Switched to ${data.provider} — applied to physical network adapters.`,
      });
    },
    onError: (e: Error) => {
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 pt-2">
        ...
        <p className="text-[11px] text-muted-foreground">
          {selectedProvider
            ? `Will apply ${DNS_OPTIONS.find(o => o.id === selectedProvider)?.name} to physical network adapters (excludes VPN/virtual)`
